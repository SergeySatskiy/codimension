...
def getUserShell():
    ...
    try:
        ...
        content = passwdFile.read()
        passwdFile.close()

        lines = content.split('\n')
        d = dict([(e.split(':', 1)[0], e.split(':')[-1]) for e in lines])
...
__osSpawn = {
    'posix'         : "%(term)s -e %(shell)s -c "
                      "'cd %(wdir)s; %(exec)s %(options)s; CDM_RES=$?; "
...
__osSpawnForProfile = {
    'posix'         : "%(term)s -e %(shell)s -c "
                      ...
                      "%(exec)s -m cProfile -o %(out)s %(options)s; "
                      "CDM_RES=$?; %(exec)s %(fb)s %(port)d $CDM_RES; "
                      "%(exit_if_ok)s %(shell)s' &",
    'Terminal'      : "Terminal --disable-server -x %(shell)s -c "
                      "'%(exec)s %(fb)s %(port)d $PPID; cd %(wdir)s; "
                      "%(exec)s -m cProfile -o %(out)s %(options)s; "
...
__osSpawnForDebug = {
    'posix'         : "%(term)s -e %(shell)s -c "
                      ...
                      "%(exec)s %(dbgclient)s %(dbgopt)s -- %(app)s; echo; "
                      "echo -e \"Script finished\"; %(shell)s' &",
    'Terminal'      : "Terminal --disable-server -x %(shell)s -c "
                      "'%(exec)s %(fb)s %(fbport)d $PPID; cd %(wdir)s; "
                      "%(exec)s %(dbgclient)s %(dbgopt)s -- %(app)s; echo; "
                      "echo -e \"Script finished\"; %(shell)s' &",
...
def getTerminalCommandToRun(fileName, workingDir, arguments,
    ...
    args = " " + prepareArguments(arguments)
    ...
    if terminalStartCmd in __osSpawn:
        return __osSpawn[terminalStartCmd] % \
                    {'shell': shell,
                     'wdir': workingDir,
                     'exec': pythonExec,
...
def getTerminalCommandToDebug(fileName, workingDir, arguments,
    ...
    if terminalStartCmd in __osSpawnForDebug:
        return __osSpawnForDebug[terminalStartCmd] % \
                    {'shell': shell,
                     'wdir': workingDir,
                     'exec': pythonExec,
                     'app': fileName + args,
                     ...
                     'dbgopt': dbgopt}
    return __osSpawnForDebug[os.name] % \
                    {'term': terminalStartCmd,
                     'shell': shell,
                     'wdir': workingDir,
                     'exec': pythonExec,
